...
//                        flower = flowerInfo.getList().get(0);
    ...
    private void showWeatherInfo(VisualizationInfo vli,Dev dev) {
        ...
        if(vli.getList()==null) getVisualizationInfo(1);
        else{
...
//        startService(intent);
    ...
    private void showWeatherInfo() {
        ...
        for(int i=0;i<6;i++){
            ...
            TextView maxText = (TextView) view.findViewById(R.id.max_text);
            TextView minText = (TextView) view.findViewById(R.id.min_text);
            dateText.setText("2018/5/5 17:19");
            infoText.setText("16℃");
            maxText.setText("65%");
            minText.setText("1万LX");
            forecastLayout.addView(view);
        }


        aqiText.setText("24.9℃");

        pm25Text.setText("350lx");
        String comfort = "";
        String carWash = "";
        String sport = "";
//        comfort = "湿度：50-85%，有条件可以使用加湿器，也可以向叶面喷水。往观花植物叶面喷水的时候切勿喷到花上，花瓣遇水容易发生病害，影响观赏效果。冬天在北方由于供暖的原因，会造成室内高温干燥，此时更应该增加空气湿度。";
...
//        sport = "sport温度：一般适宜花卉生长的温度15-35℃，观叶植物在开花期，应该在保证不发生冻害的前提下尽量降低温蒂，降低温度可以延长花期。";
        ...
        try {
            comfort = flowerInfo.getList().get(0).getIntro();
            carWash = flowerInfo.getList().get(0).getPlantmehod();
            sport = flowerInfo.getList().get(0).getHabit();
        } catch (Exception e) {
            e.printStackTrace();
        }
        comfortText.setText(comfort);
        ...
        intent.putExtra("user_data",user);
        startService(intent);
        Toast.makeText(WeatherActivity.this,"111111111111",Toast.LENGTH_LONG);
